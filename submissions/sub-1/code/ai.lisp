...
(let (
  ...
  (asupdate (lambda (l x y) (cons (cons x (add y (call asfind asfind l x))) l)))
  (visited 0)
  (adjghost (lambda (f x y g) (if (atom g) 0
    (let ((gx (car (car (cdr (car g)))))
          (gy (cdr (car (cdr (car g)))))
          (d 0))
      (set d (add (call abs (sub x gx)) (call abs (sub y gy))))
      (if (gt 2 d) 1 (call f f x y (cdr g))))))))
(let (
  (dirtest (lambda (map x y dir g)
    (let ((nx (sub (add x (call at at dx dir)) 1))
      ...
      (if (eq 0 (call mapat map nx ny)) 0
        (if (call adjghost adjghost nx ny g) 0 1))))))
(let (
